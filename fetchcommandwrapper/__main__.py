...
def gentoo_mirrors():
    ...
    p = subprocess.Popen(
        ["/usr/bin/portageq", "gentoo_mirrors"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def supported(uri):
    return uri.startswith("http://") or uri.startswith("https://") or uri.startswith("ftp://")
...
def _inner_main():
    ...
    if not os.path.isdir(opts.distdir):
        print('ERROR: Path "%s" not a directory' % opts.distdir, file=sys.stderr)
        sys.exit(1)

    if opts.continue_flag:
        if not os.path.isfile(opts.file_fullpath):
            print('ERROR: Path "%s" not an existing file' % opts.file_fullpath, file=sys.stderr)
            sys.exit(1)
